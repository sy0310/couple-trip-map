...
import { getCoupleId, getVisitedProvinces, getVisitedCitiesWithCoords, getAllPhotosForCouple, getCoupleInfo, getTimelines } from "@/lib/trips";
...
export default function StatsPage() {
  ...
  useEffect(() => {
    getCoupleId().then((cid) => {
      if (!cid) return;
      Promise.all([getVisitedProvinces(cid), getVisitedCitiesWithCoords(cid), getAllPhotosForCouple(cid), getCoupleInfo(), getTimelines(cid)])
        .then(([p, c, ph, ci, tls]) => {
          setProvinces(p); setCities(c); setAllPhotos(ph);
          if (ci?.sinceDate) {
            const days = Math.floor((Date.now() - new Date(ci.sinceDate).getTime()) / 86400000);
            setDaysSince(days);
            const d = new Date(ci.sinceDate);
            setSinceDateLabel(`${d.getFullYear()}年${d.getMonth() + 1}月${d.getDate()}日`);
          }
          setTimelines(tls);
        });
    });
      ...
      {activeTab === "timeline" && (
        <div style={{ padding: "0 16px" }}>
          <div style={{ background: `linear-gradient(135deg, ${T.accent}15, ${T.accentFaint})`, borderRadius: 20, padding: "24px 20px", textAlign: "center", border: `1.5px solid ${T.accent}30`, marginBottom: 20, boxShadow: T.shadow }}>
            ...
            <div style={{ fontFamily: "var(--font-noto-serif-sc)", fontWeight: 700, fontSize: 22, color: T.ink }}>{sinceDateLabel || "未设置"}</div>
          ...
          <div style={{ position: "relative", paddingLeft: 24, paddingBottom: 20 }}>
            ...
            {timelines.map((tl) => {
              const isPast = new Date(tl.date) <= new Date();
              return (
              <div key={tl.id} style={{ position: "relative", marginBottom: 20 }}>
                <div style={{ position: "absolute", left: -18, top: 10, width: 14, height: 14, borderRadius: 7, background: isPast ? T.accent : T.bgCard, border: `2px solid ${T.accent}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 7, color: "white" }}>{isPast ? "✓" : ""}</div>
                <div style={{ background: T.bgCard, borderRadius: 14, border: `1px solid ${isPast ? T.accent + "30" : T.border}`, padding: "14px 16px", opacity: isPast ? 1 : 0.6, boxShadow: T.shadow }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 18 }}>{tl.icon || "✦"}</span>
                    <span style={{ fontFamily: "var(--font-noto-serif-sc)", fontWeight: 700, fontSize: 16, color: T.ink }}>{tl.title}</span>
                    {!isPast && <Pill small>即将到来</Pill>}
                  </div>
                  <div style={{ fontSize: 11, color: T.accent, marginBottom: 6, fontWeight: 500 }}>{tl.date}</div>
                  {tl.description && <div style={{ fontSize: 12, color: T.inkMid, fontStyle: "italic", lineHeight: 1.5 }}>{tl.description}</div>}
                </div>
              </div>
              );
            })}
